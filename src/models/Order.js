...
const subVariantSchema = new mongoose.Schema({
  ...
  subVariantRegularPrice: { type: Number, default: 0 },
  subVariantDiscountPrice: { type: Number, default: 0 },
...
const deliveryServiceSchema = new mongoose.Schema({
  ...
  pickedUpDate: { type: Date, default: null },
  courierResponse: { type: mongoose.Schema.Types.Mixed, default: {} },
...
const orderSchema = new mongoose.Schema({
  ...
  // ✅ paymentStatus now includes 'partial'
  paymentStatus: {
    type: String,
    enum: ['pending', 'paid', 'failed', 'refunded', 'partial'],
    default: 'pending'
  },
  // ✅ NEW: Amount actually paid/collected (auto-computed for partial/COD)
  paidAmount: { type: Number, default: 0, min: 0 },
  // ✅ NEW: Amount to refund (returned items value when payment was already collected)
  refundableAmount: { type: Number, default: 0, min: 0 },
  // ✅ NEW: Snapshot of returned amount in last partial delivery update
...
orderSchema.methods.initializeDeliveryItems = function () {
  ...
  this.items.forEach((item) => {
    ...
    // Helper to compute unit price for a line
...
// ============================================================
// ✅ NEW: Compute returned amount from deliveryItems
// ============================================================
...
// ============================================================
// ✅ NEW: Compute delivered amount from deliveryItems
// ============================================================
...
// ============================================================
// ✅ NEW: Recompute payment status & amounts based on delivery state
//      Rules:
//        - Only applies when order is (or becomes) partial_delivery or delivered
//        - paidAmount = subtotal_from_delivered_items + shipping - discount
//                       (clamped to [0, total])
...
orderSchema.methods.recomputePaymentFromDelivery = function () {
  ...
  // Handle shipping & discount when nothing has been delivered
  // Shipping is only charged if at least one item is delivered.
  // Discount is scaled proportionally to what is delivered.
  ...
  // Don't downgrade a manually marked 'refunded' state
...
orderSchema.virtual('formattedOrderNumber').get(function () {
  if (this.orderNumber) {
    const match = this.orderNumber.match(/^BB(\d+)$/);
    if (match) return `BB-${match[1].padStart(4, '0')}`;
  }
  return this.orderNumber;
});
